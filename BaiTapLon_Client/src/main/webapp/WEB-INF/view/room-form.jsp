...
<body>
	...
	<h1>Room</h1>
	<c:url var="saveUrl" value="/room/saveRoom?categoryId=${categoryId}" />
	<c:if test="${isUpdate}">
		<c:url var="saveUrl" value="/room/updateRoom?roomId=${roomId}" />
	</c:if>
		...
		<div class="col-lg-6">
			<div class="card">
				<div class="card-body">
					...
					<form:form modelAttribute="roomAttribute" method="POST"
						action="${saveUrl}">
						<form:hidden path="maPhong" value="${room.maPhong}" />
						<%-- <input:hidden path="maLoai" value="${categoryId}" /> --%>
						<table>
							<tr>
							<tr>
							...
							<tr>
								<td><form:label path="tinhTrang">Tình trạng:</form:label></td>
								<td><form:radiobutton checked="checked" path="tinhTrang"
										value="1" label="Trống" /> <form:radiobutton path="tinhTrang"
										value="2" label="Đã đặt" /> <form:radiobutton
										path="tinhTrang" value="3" label="Đang dọn" /></td>
							</tr>
						</table>
